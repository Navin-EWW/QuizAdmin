...
import { debounce, String } from "lodash";
import moment from "moment";
import { useCallback, useState } from "react";
import { UserLogList } from "../../../api/merchant/user";
...
export default function UserLog({ state }: any) {
  ...
  const { dataUpdatedAt, error, isError, isLoading, isFetching, refetch } =
    useQuery(
      ["userLog"],
      () => UserLogList({ userId, per_page, current_page, sortBy, sortType }),
      {
    ...
    <>
      <div className="pb-20">
        <div className="px-5 bg-white shadow-md rounded-b-md">
                ...
                >
                  ...
                  >
                    <div className="flex items-center gap-2">
                      ...
                      <span
                        className="cursor-pointer"
                        onClick={() => setsortDiscription(!sortDiscription)}
                      >
                        <a onClick={() => SortLog("description")}>
                          {sortBy === "description" ? (
                            sortType === "asc" ? (
                              <img src={DownArrow} />
                            ) : (
                ...
                {services?.map((service, index) => {
                  ...
                  return (
                    <tr key={index}>
                      ...
                      {/* <td className="whitespace-nowrap px-3 py-4">#JP0390218</td> */}
                      <td className="whitespace-nowrap px-6 py-4">
                        {service.logType}
                      </td>
                      <td className="whitespace-nowrap px-6 py-4">
          ...
          <div className="flex items-center justify-between bg-white py-3 border-t border-grey_border_table font-Inter pb-4">
