...
import { MerchantListStatus } from "../../../api/merchant/merchant";
...
const ToggleModel = ({
  ...
  const { mutate } = useMutation(MerchantListStatus, {
    onSuccess: (data: any) => {
    ...
    <Transition.Root show={open} as={Fragment}>
      <Dialog as="div" className="relative z-10" onClose={setOpen}>
        ...
        <div className="fixed inset-0 z-10 overflow-y-auto">
          <div className="flex rounded-md min-h-full items-center justify-center p-4  sm:p-0">
            <Transition.Child
              as={Fragment}
              enter="ease-out duration-300"
            ...
            >
              <Dialog.Panel className="relative rounded-md font-Nunito transform overflow-hidden bg-white px-4 pt-5 pb-4 text-left shadow-xl transition-all sm:my-8 sm:w-full sm:max-w-lg sm:p-6">
                ...
                <div className="mt-5 sm:mt-4 sm:flex gap-3">
                  <button
                    type="button"
                    className={`inline-flex w-full rounded-md justify-center border border-transparent ${
                      status === "ACTIVE"
                        ? "bg-blue_primary hover:bg-hoverChange"
                        : "bg-error_red"
                    } px-4 py-2 text-base font-normal text-white shadow-sm focus:outline-none`}
                  ...
                  <button
                    type="button"
                    className="mt-3 inline-flex rounded-md w-full justify-center border border-grey_border bg-white hover:bg-grey_bg px-4 py-2 text-base font-normal text-font_dark shadow focus:outline-none sm:mt-0"
                    onClick={() => setOpen(false)}
                  >
                    Cancel
                  </button>
