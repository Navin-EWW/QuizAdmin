...
import { AdminLogList } from "../../../api/admin/admin";
...
export default function Subjectlog({ state }: any) {
  const AdminId = state;
  ...
  const { dataUpdatedAt, error, isError, isLoading, isFetching, refetch } =
    useQuery(
      ["adminlog"],
      () => AdminLogList({ AdminId, per_page, current_page, sortBy, sortType }),
      {
  ...
  // const SortLog = (logdata: String, SortType: String) => {
  //   setLogType(logdata);
