...
type DataProps = {
  ...
  error?: string | null;
  id?: string | null;
  Crosscss?: string | null;
  Panelcss?: string | null;
  Headercss?: string | null;
  subHeaderCss?: string | null;
      ...
      <Dialog as="div" className="relative z-[100]" onClose={setOpen}>
        ...
        <div className="fixed inset-0 z-10 overflow-y-auto">
          <div className="flex min-h-full items-center justify-center p-4 sm:p-0">
            ...
            >
              <Dialog.Panel
                className={`relative rounded-[8px]  transform overflow-hidden bg-white ${
                  Panelcss ? Panelcss : "px-4 pt-5 pb-4 sm:px-6 sm:py-5"
                } text-left shadow-xl transition-all sm:my-8 sm:w-full sm:max-w-[480px] `}
              >
                <div
                  className={`absolute ${
                    Crosscss ? Crosscss : "top-0 right-0 pt-5 pr-5"
                  } block`}
                >
                  <button
                    type="button"
                ...
                <div className="">
                  <div className=" sm:mt-0 text-left">
                    <Dialog.Title
                      as="h3"
                      className={`${
                        Headercss ? Headercss : "text-sm font-bold uppercase  "
                      } leading-6 text-font_black`}
                    >
                      {header}
                    </Dialog.Title>
                    {subHeader && (
                      <div className="mt-2 border-t border-grey_border py-4">
                        <p
                          className={`${
                            subHeader
                              ? subHeaderCss
                              : "text-base text-font_black font-normal"
                          }`}
                        >
                          {error && (
                ...
                <div className="sm:mt-2 sm:flex gap-3 gap-y-2">
                  {buttonlevelblue && (
                    <button
                      type="button"
                      className="inline-flex w-full rounded-md justify-center border border-transparent bg-blue_primary hover:bg-hoverChange px-4 py-2 text-base font-normal text-white shadow-sm focus:outline-none"
                      onClick={confirmClicked || ErrorHndler}
                    >
                      {buttonlevelblue}
                    </button>
                  )}
