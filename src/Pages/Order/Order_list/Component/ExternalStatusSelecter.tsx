...
export default function ExternalStatusSelecter(prop: Props) {
  const {
    selectedValue = [],
    AllData = [],
    isOpen,
  ...
  const handleClickOutside = (event: any) => {
    if (event.target.closest("#list-box-external, #list_btn-external")) return;
    else {
      setIsOpen(false);
    }
  };
  ...
  return (
    <div>
      ...
      >
        {() => (
          <>
            <span className="inline-block w-full" id="list_btn-external">
              <Listbox.Button
                className="inline-flex w-full justify-between items-center border border-grey_border_table bg-gray-50 rounded-md px-[13px] py-1.5 text-sm font-medium text-grey_border outline-none transition ease-in-out duration-150"
              ...
              >
                {selectedValue.length < 1 ? (
                  ...
                  <span className="block capitalize text-xs text-font_black truncate">
            ...
            >
              <div id="list-box-external" className="border-grey_border">
