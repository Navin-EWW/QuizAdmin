...
export default function LastMileSelector(prop: Props) {
  const {
    selectedValue = [],
    AllData = [],
    isOpen,
  ...
  const [update, setUpdateOnClick] = useState<boolean>(false);
  let labelid;
  ...
  const handleClickOutside = (event: any) => {
    if (event.target.closest("#list-box-lastmile, #list_btn-lastmile")) return;
    else {
      setIsOpen(false);
    }
  };
  ...
  return (
    <div onClick={divOnClick}>
      ...
      >
        {() => (
          <>
            <span className="inline-block w-full" id="list_btn-lastmile">
              <Listbox.Button
                className={`inline-flex  w-full justify-between items-center border border-grey_border_table bg-gray-50 rounded-md ${
                  extraCss ? extraCss : "py-1.5"
                } px-[13px] text-sm font-medium text-grey_border outline-none transition ease-in-out duration-150`}
            ...
            >
              <div
                id="list-box-lastmile"
                className="border-grey_border max-w-xs"
              >
