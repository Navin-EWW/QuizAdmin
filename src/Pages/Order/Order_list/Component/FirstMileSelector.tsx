...
export default function FirstMileSelector(prop: Props) {
  const {
    selectedValue = [],
    AllData = [],
    isOpen,
  ...
  const handleClickOutside = (event: any) => {
    if (event.target.closest("#list-box-first, #list_btn-first")) return;
    else {
      setIsOpen(false);
    }
  };
  ...
  return (
    <div>
      ...
      >
        {() => (
          <>
            <span className="inline-block w-full" id="list_btn-first">
              <Listbox.Button
            ...
            >
              <div id="list-box-first" className="border-grey_border max-w-xs">
