...
export default function InternationalMileSelector(prop: Props) {
  const {
    selectedValue = [],
    AllData = [],
    isOpen,
  ...
  const handleClickOutside = (event: any) => {
    if (
      event.target.closest("#list-box-international, #list_btn-international")
    )
      return;
    else {
      setIsOpen(false);
  ...
  return (
    <div>
      ...
      >
        {() => (
          <>
            <span className="inline-block w-full" id="list_btn-international">
              <Listbox.Button
